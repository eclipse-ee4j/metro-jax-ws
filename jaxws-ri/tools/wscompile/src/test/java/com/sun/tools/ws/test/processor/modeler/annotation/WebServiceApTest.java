...
public class WebServiceApTest extends TestCase {
    ...
    public void testRemoteIfaceArg() {
        ...
        JavaCompiler.CompilationTask task = getCompilationTask("com.sun.tools.ws.test.processor.modeler.annotation.RemoteArgTestWS",
    ...
    public void testRemoteIfaceReturn() {
        ...
        JavaCompiler.CompilationTask task = getCompilationTask("com.sun.tools.ws.test.processor.modeler.annotation.RemoteReturnTestWs",
                options, wsgenOptions, diagnostics);
        try {
            task.call();
        } catch (RuntimeException re) {
    ...
    private boolean runCompiler(List<String> args, WebServiceAp ap) {
        ...
        Iterable<? extends JavaFileObject> compilationUnits = new HashSet<JavaFileObject>() {
            {
                add(new JavaSourceFromString("com.sun.tools.ws.test.processor.modeler.annotation.EndpointJavaSource"));
            }
        };
        JavaCompiler.CompilationTask task = compiler.getTask(
                null,
